...
static int get_commandline_parameters(int argc, char *argv[], char **ics_filename,
   ...
   char **ct, char **cd, float *desired_resolution, int *use_all_regions, int *use_labels, int *fileformat, char **execution_path);
...
printf(">>>>>>>>>>>>>>>>>>>>>>>>>>");
   ...
   ****************************************************************************/
   status = get_commandline_parameters(argc, argv, &ics_filename,
               ...
               &use_all_regions, &use_labels, &fileformat, &path);
   if(status == 0) exit(1);
   ...
   ****************************************************************************/
   ...
   if(overlay_filename != NULL){
      char * complete_file_path = malloc(512);
        
      //fprintf(stdout, "\n overlay_filename : %s", overlay_filename);
      sprintf(complete_file_path, "%s/%s", path, overlay_filename);
        fprintf(stdout, "\n overlay_filename : %s \n", complete_file_path);
      status = read_overlay_file(complete_file_path, &overlay_data);
      if(status == 0) exit(1);
